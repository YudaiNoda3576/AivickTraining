...
public class TreeInRowAns {
	private static List<Cell> cells = List.of(Cell.empty(), Cell.empty(), Cell.empty(), Cell.empty(), Cell.empty(),
	...
	public static void main(String[] args) {
		...
		Player humamPlayer = new HumanPlayer(sc, firstSecond.equals("1") ? CellStatus.BATSU : CellStatus.MARU);
		Player computerPlayer = new ComputerPlayer(firstSecond.equals("2") ? CellStatus.BATSU : CellStatus.MARU);
		...
		while (true) {
			...
			place(cellPlace, player.maruBatsu() == CellStatus.BATSU ? CellStatus.BATSU : CellStatus.MARU);
	...
	private static boolean isLine(int one, int two, int three) {
		return cells.get(one).status() != CellStatus.NONE && cells.get(one).status() == cells.get(two).status()
				&& cells.get(two).status() == cells.get(three).status();
	}
	...
	public int doTurn(List<Cell> cells) {
		while (true) {
			try {
				...
				if (place >= 0 && place < 9 && cells.get(place).status() == CellStatus.NONE) {
					return place;
				}
			} catch (Exception e) {
...
class ComputerPlayer implements Player {
	...
	private static int nextPlace(List<Cell> cells) {
		for (int i = 0; i < cells.size(); i++) {
			if (cells.get(i).status() == CellStatus.NONE) {
				return i;
			}
		}
		return -1;
